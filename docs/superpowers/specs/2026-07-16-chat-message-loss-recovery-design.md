...
- 덩어리 2와 **동일 조건**으로 재측정 → **유실률 0%**(재현 조건에서 유실 0건 관측) +
  재연결 타임스탬프 분산 확인.
- **`docs/private/records/6-chat.md`** — **나중에 문제/해결/성과로 축약하기 좋도록
  원재료를 구체적이고 풍부하게** 남기는 raw 기록. portfolio-writing 스킬의 틀
  (제목/그림/문제/해결/성과, 문장 형식 등)을 여기 적용하지 않는다 — 축약은 나중
  (9단계) 일이고, 이 문서는 그때 뽑아 쓸 자료를 자세히 모아두는 게 목적. 아래를
  구체적으로 기록:
  - **문제 배경**: 왜 소켓 기본(at-most-once)이 유실을 내는지, 원래 구조가 왜
    그랬는지(당시 타당했던 선택), 방치 시 영향.
  - **재현(장애 가정) 방법**: 어떤 스크립트로·어떤 조건(kill 타이밍·메시지 rate·
    클라이언트 수)에서 재현했는지, 스크립트 경로·실행법.
  - **해결 과정과 방법**: 세 구멍 각각을 어떤 기술·설계로 풀었는지 구체적으로,
    "CSR 대신 DB 재동기화를 택한 판단 근거"(RESUME_PLAN FE-2 면접 방어 2단 반박),
    시행착오·대안 검토.
  - **전후 수치**: 재현 조건 명시 + 유실률 N%→0%, 재연결 타임스탬프 분산, 측정 로그.
  - **실동작 증거**: 스크린샷/녹화/로그.
  - "장애 가정" 표현은 최종 포트폴리오 글에서 금지되는 것이고, records엔 재현 방법을
    사실대로 자세히 적어둔다.
